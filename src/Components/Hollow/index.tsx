...
function Hollow(props:IProps){

    const myRef: RefObject<HTMLDivElement> = useRef<null | HTMLDivElement>(null);
    const [soursePosition, setSoursePosition] = useState<IPosition[]>([])
    let currentPosition = soursePosition;
    console.log(soursePosition)
    useEffect(()=>{
        if(myRef.current !== null) {
            const initialDataCard:IPosition[] = Object.values(myRef.current.children[0].children).map((item:any)=>{
                return {
                    id:item.id,
